...
sealed trait Process[+F[_], +O]
  ...
  final def onHalt[F2[x] >: F[x], O2 >: O](f: Cause => Process[F2, O2]): Process[F2, O2] = {
     ...
     this match {
       case append: Append[_, _] => Append[F2, O2](append.head.asInstanceOf[HaltEmitOrAwait[F2, O2]], (append.stack :+ next).asInstanceOf[Vector[Cause => Trampoline[Process[F2, O2]]]])
       case emt@Emit(_)        => Append[F2, O2](emt, Vector(next))
       case awt: Await[_,_,_]  => Append[F2, O2](awt.asInstanceOf[HaltEmitOrAwait[F2, O2]], Vector(next))
  ...
  final def unconsOption[F2[x] >: F[x], O2 >: O](implicit F: Monad[F2], C: Catchable[F2]): F2[Option[(O2, Process[F2, O2])]] = step match {
    case Step(head, next) => head match {
      ...
      case await: Await[_, _, _] => await.asInstanceOf[Await[F2,_,O2]].evaluate(F, C).flatMap(p => (p +: next).unconsOption(F,C))
    }
