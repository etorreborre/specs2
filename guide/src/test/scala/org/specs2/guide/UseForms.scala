...
object UseForms extends UserGuidePage with Forms {
  def is = "Forms".title ^ s2"""
...
When the form is displayed, here is how the fields are displayed:
  ...
  ${form("Fields")
    ...
    .tr(
      field("field(\"label\", field(\"value1\"), field(\"value2\"))").code,
      field("label", field("value1"), field("value2"))
    )}
...
Let's look at a few examples:
 ...
 ${form("Properties")
    ...
    .tr(
      field("prop(\"label\", { error(\"but got an error\"); \"actual\" })(\"expected\")").code,
      prop("label", { error("but got an error"); "actual" })("expected")
    )
    .tr(
      field("prop(\"label\", \"actual\", (a: String, b: String) => (a === b))(\"expected\")").code,
      prop("label", "expected", (a: String, b: String) => (a === b)).apply("expected")
    )
    .tr(
      field("prop(\"label\", \"actual\", (s: String) => beEqualTo(s))(\"expected\")").code,
      prop("label", "expected", (s: String) => beEqualTo(s))("expected")
    )
    .tr(
      field("prop(\"label\", \"actual\", beEqualTo(\"expected\"))").code,
      prop("label", "actual", beEqualTo("expected"))
    )
    .tr(
      field("prop(\"label\", \"actual\", beEqualTo(\"expected\").mute)").code,
      prop("label", "actual", beEqualTo("expected").mute)
    )
    .executeForm
    .toXml
    .toString}
...
Most of the time, the display of Fields and Properties can be left as it is but sometimes you want to style the output of labels and values. You can do this by using `decorateWith` and `styleWith` methods, or some equivalent shortcuts:
 ...
 ${form("Style")
    ...
    .tr(
      field("field(\"label\", \"value\").decorateWith(f: Any => <em>{f}</em>)").code,
      field("label", "value").decorateWith((ns: Any) => <em>{ns}</em>)
    )
    ...
    .tr(
      field("field(\"label\", \"value\").styleWith(\"color\"->\"#FF1493\")").code,
      field("label", "value").styleWith("color" -> "#FF1493")
    )
...
${Form("Person").tr(field("name", "Eric")).tr(address.inline)}
...
${Form("purchase")
    .tr(loginForm.toEffect("login"))
    .tr(selectForm.toEffect("select goods"))
    .tr(checkKoTotalForm.toProp("the total must be computed ok").bkWhiteLabel)
    .executeForm
    .toXml
    .toString}
...
${Calculator.th("a", "b", "a + b", "a - b").tr(1, 2, 3, -1).tr(2, 2, 4, 0)}

And if something goes wrong:

${Calculator.th("a", "b", "a + b", "a - b").tr(1, 2, 3, -1).tr(2, 2, 4, 2).form.executeForm.toXml.toString}

And when it goes *very* wrong (like throwing an `error("very wrong")`), there will be red cells and stacktraces:

${WrongCalculator.th("a", "b", "a + b", "a - b").tr(1, 2, 3, -1).tr(2, 2, 4, 2).form.executeForm.toXml.toString}
...
"""
  ...
  class CalculatorSpecification extends Specification with specification.Forms {
    def is =
  ...
  lazy val addresses =
    Seq(ComponentsDefinitions.Address("Rose Crescent", 3), ComponentsDefinitions.Address("Oxfort St", 4))
  ...
  val checkTotalForm = Form("Check Total").tr(effect("Compute total", 100)).tr(prop("Total", 100)(100))

  val checkKoTotalForm = Form("Check Total").tr(effect("Compute total", 100)).tr(prop("Total", 100)(200))
...
object Model:
  case class Address(street: String = "street", number: Int = 1):
    def retrieve(addressId: Int) =
      val address = Address()
      Form("Address").tr(prop("street", address.street, street)).tr(prop("number", address.number, number))
  ...
  case class Customer(name: String = "", address: Address = Address()):
    def retrieve(customerId: Int) =
      val customer = actualCustomer(customerId)
      Form("Customer").tr(prop("name", customer.name)(name)).tr(address.actualIs(customer.address))
