...
You will need however to annotate your specification classes as JUnit tests (this requires the `specs2-junit` jar):${snippet{
...
With Gradle you need to use the same `RunWith` annotation and the [`test`](http://www.gradle.org/docs/current/dsl/org.gradle.api.tasks.testing.Test.html) task.
You can also follow the instructions in [this blog post](http://blog.mindcrime-ilab.de/2013/10/25/gradle-rocking-scala-specs2-tests) and create a task that will leverage the $specs2 ${"file runner" ~/ RunInShell}. This way you will avoid having to annotate the classes.
"""
}
