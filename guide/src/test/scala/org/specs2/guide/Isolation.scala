...
Unit specifications allow to nest blocks, as if they were different contexts, going from the more general to the more specific. It can be very tempting, for some kind of applications, to include some mutable state representing more and more elaborated data. This is problematic however because:

 - you have to reset state for a sequential specification
 - you will have concurrency issues for a parallel one
...
_Note_: if the Specification is defined has a constructor with parameters, they need themselves to be instantiable with a no-args constructor. For example `class AkkaSpec(system: AkkaSystem) extends Specification` can not be instantiated because `AkkaSystem` doesn't have a 0-args constructor.
