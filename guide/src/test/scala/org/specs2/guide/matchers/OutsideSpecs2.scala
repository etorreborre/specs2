...
When there is a failure or an error the library will call the `TestKit` methods. Then the library client can use both the library and $specs2 by mixing in the `${fullName[
    matcher.ThrownMessages
  ]}` trait:
```
