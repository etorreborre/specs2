...
Instead of commenting them out and then forgetting about those examples when the code is fixed, you can append `pendingUntilFixed` to the example: ${snippet{
...
The example above will be reported as `Pending` until it succeeds. \
Then it is marked as a failure so that you can remember to remove the `pendingUntilFixed` marker.
"""
}
