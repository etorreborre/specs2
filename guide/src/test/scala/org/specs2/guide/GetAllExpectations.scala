...
object GetAllExpectations extends UserGuidePage {
  def is = s2"""
...
This enables a type of specification where it is possible to define lots of expectations inside the body of an example and get a maximum of information on what fails and what passes: ${snippet {
...
"Mutable" means that the concurrent execution of examples will be an issue if done blindly. To avoid this, the `AllExpectations` trait overrides the `Specification` arguments to make it ${"sequential" ~/ Execution} (unless it already is).
