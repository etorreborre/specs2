...
object AutoExamples extends UserGuidePage { def is = s2"""

 Some specifications are just about giving examples on using an API:${snippet{
...
It would be a bit redundant to provide a textual description for these 3 examples because the code is pretty clear and simple. In this situation you can use the `eg` operator to create an example where the description will be the code itself:
```
...
There is a huge gotcha though! Each of these expressions needs an implicit conversion to be included in the interpolated spec. And in Scala, if you have a block of code returning a value of type `T`, ***only the last expression of the block is converted***. This means that if there is a statement in the block that throws an exception this exception won't be caught and the whole specification will fail to be built! So if you want to use blocks as auto-examples you should better wrap them with an `eg` call:${snippet{
class SeqSpecification extends Specification { def is = s2"""
