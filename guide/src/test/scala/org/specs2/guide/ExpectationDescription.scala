...
You can improve this failure message by describing what `ticketsNumber` represents with the `aka` (*also known as*) method : ${snippet{
val ticketsNumber = 5
...
And there are some variations on the `aka` method:
...
In the example above, testing if `parameter == "evaluated"` is just a way to observe what we wanted to achieve. If that doesn't work, the failure message will be "The byname function has not become a strict one because 'evaluated' is not equal to 'not evaluated'".
"""
}
