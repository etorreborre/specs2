 ...
 * `beSome.like(partial function)` check if an element is `Some(_)` and satisfies a partial function returning a `Result`
 ...
 * `beRight.like(partial function)` check if an element is `Right(_)` and satisfies a partial function returning a `Result`
 ...
 * `beLeft.like(partial function)` check if an element is `Left(_)` and satisfies a partial function returning a `Result`
  """
}
