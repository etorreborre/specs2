...
class FragmentsApi extends UserGuidePage { def is = "Fragments API".title^
  """
### Presentation

The interpolated string shown in the [Quick Start](org.specs2.guide.QuickStart.html) is actually desugared to a list of `Fragment` objects joined by the `^` operator:
