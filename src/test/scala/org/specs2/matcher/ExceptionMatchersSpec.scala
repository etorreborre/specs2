...
class ExceptionMatchersSpec extends SpecificationWithJUnit { def is =
  ...
  def e10 = (theBlock(throw UserError("me", "boom")) must throwAn(UserError("me2", "boom")).
		    like { case UserError(name, _) => name.endsWith("2") }).message must endWith("boom instead")

  def e11 = (theBlock(throw UserError("me", "boom")) must throwAn(UserError("me2", "boom")).
		     like { case UserError(name, _) => name.startsWith("m") }).message must beMatching("Got the exception .*") 
