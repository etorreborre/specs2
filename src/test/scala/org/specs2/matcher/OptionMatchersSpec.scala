...
class OptionMatchersSpec extends Specification with ResultMatchers { def is = s2"""
 ...
 The OptionMatchers trait provides matchers to check Option instances.
  ...
  ${ (Some(1) must beSome.which(_ > 0)) returns "'Some(1)' is Some and satisfies the given predicate" }
  ${ (Some(1) must beSome.which(_ < 0)) returns "'Some(1)' is Some but 1 does not satisfy the given predicate" }
