...
class Location {
  ...
  private def fragmentFilter = (st: Seq[StackTraceElement]) => {
    ...
    // to find out the exact location of a fragment
      ...
      // 164 is the magic line number of the "FragmentsFragment::def ^(t: String) = fragments add Text(t)" method
      if (lastContains(start, "$up(FragmentsBuilder.scala:164") && !isTextStart(start)) takeHead(end, st(0), lineOffset = 1)
