...
trait Notifier {
  ...
  def exampleFailure(name: String, message: String, location: String, f: Throwable, duration: Long)
...
object ConsoleNotifier extends Notifier {
  ...
  def exampleFailure(name: String, message: String, location: String, f: Throwable, duration: Long) = Console.println(Seq("exampleFailure",name   , message,location,f.getMessage, duration).mkString("; "))
