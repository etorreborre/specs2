...
object depends {
  ...
  def scalaz(scalazVersion: String) =
    Seq("org.scalaz"        %% "scalaz-core"      ,
      ...
      (if (scalazVersion.startsWith("7.1")) Seq("org.scalaz.stream" %% "scalaz-stream" % "0.6a")
       else                                 Seq("org.scalaz.stream" %% "scalaz-stream" % "0.6"))

  lazy val scalacheck    = Seq("org.scalacheck" %% "scalacheck"   % "1.12.1")
