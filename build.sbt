...
lazy val core = crossProject(platforms: _*)
  ...
  .crossType(CrossType.Pure)
  .in(file("core"))
  .settings(name := "specs2-core", commonSettings, libraryDependencies += depends.junit % Test)
  .jvmSettings(commonJvmSettings)
  .jsSettings(commonJsSettings)
...
lazy val junit = crossProject(platforms: _*)
  ...
  .crossType(CrossType.Pure)
  .in(file("junit"))
  .settings(name := "specs2-junit", commonSettings, libraryDependencies += depends.junit)
  .jvmSettings(commonJvmSettings)
  .jsSettings(commonJsSettings)
...
lazy val releaseSettings: Seq[Setting[_]] = Seq(
  ...
  ThisBuild / githubWorkflowPublish := Seq(
    ...
    WorkflowStep.Use(
      ...
      params = Map("branch" -> "gh-pages", "clean" -> "false", "folder" -> "guide/target/specs2-reports/site")
    )
  ),
  organization := "org.specs2",
