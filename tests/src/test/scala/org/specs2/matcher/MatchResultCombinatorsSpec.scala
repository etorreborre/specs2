...
class MatchResultCombinatorsSpec extends mutable.Spec with ResultMatchers with MatchResultCombinators with TypedEqual:
  ...
  "Match results can be combined with and" >> {
    (1 must ===(1)) and (2 must ===(2))
    1 must be_==(1) and be_==(1)

    ((1 must ===(2)) and (2 must ===(2))) must throwA[MatchFailureException[Int]]
    ...
    ((1 must ===(2)) and (2 must ===(2))) must beFailing
    (1 must ===(1)) and success
  }
  ...
  "Match results must not be evaluated twice when failing with and" >> {
    "when the first match is failing" >> {
      ...
      {evaluated += 1; 1 must ===(2)} must beFailing
      evaluated === 1
    }
  ...
  "Match results can be combined with or" >> {
    (1 must ===(2)) or (2 must ===(2))
    (1 must ===(1)) or (1 must ===(2)) or (1 must ===(3))
    ((1 must ===(2)) or (1 must ===(3)) or (1 must ===(4))) must throwA[MatchFailureException[Int]]
    ((1 must ===(2)) or (2 must ===(3))) must beFailing
    (1 must ===(2)) or success
  }

  "A match result can be negated" >> {
    (1 must ===(2)).not
    (1 must ===(1)).not must beFailing
    (1 must ===(2)).not must beSuccessful
  }

  "A match result can be evaluated only when a boolean condition is satisfied" >> {
    (1 must ===(2)).when(false)
    (1 must ===(2)).when(true) must throwAn[Exception]
  }

  "A match result can be evaluated only unless a boolean condition is satisfied" >> {
    (1 must ===(2)).unless(true)
    (1 must ===(2)).unless(false) must throwAn[Exception]
  }

  "A match result can be evaluated if and only if a boolean condition is satisfied" >> {
    (1 must ===(2)).iff(false)
    (1 must ===(1)).iff(true)
    (1 must ===(2)).iff(true) must throwAn[Exception]
    (1 must ===(1)).iff(false) must throwAn[Exception]
  }
  ...
  "MatchResult combinators work with the AllExpectations trait" >> {
    ...
    stored.map(_.message).head must ===("'hello world' doesn't have size 10 but size 11")
  }
