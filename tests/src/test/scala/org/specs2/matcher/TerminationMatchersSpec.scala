...
class TerminationMatchersSpec extends script.Specification with TerminationMatchers with Grouped { def is = section("travis") ^ sequential ^ s2"""
 ...
 We should be able to observe that an action unblocks another
  ...
  "termination" - new group {
    eg := { implicit ee: EE => Thread.sleep(50) must terminate(sleep = 200.millis) }
    eg := { implicit ee: EE => (Thread.sleep(300) must terminate(retries=1, sleep=100.millis)) returns "the action is blocking with retries=1 and sleep=100" }

    eg := { implicit ee: EE => Thread.sleep(50) must terminate(retries=3, sleep=20.millis) }
    eg := { implicit ee: EE => (Thread.sleep(1000) must terminate(retries=3, sleep=20.millis)) returns "the action is blocking with retries=3 and sleep=20" }
    ...
    eg := { implicit ee: EE => Thread.sleep(150) must not terminate }
    ...
    eg := { implicit ee: EE =>
      ...
      var stop = true
      def action1() = scalaz.concurrent.Future({ while (stop) { Thread.sleep(10)}; queue1.add(1) }).run
