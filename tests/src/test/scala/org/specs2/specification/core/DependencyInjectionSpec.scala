...
"""
  ...
  case class Spec3()(using ec: ExecutionContext) extends Specification { def is = "test" ! Await.result(Future(ok), 1 second) }
  case class Spec4()(using es: ExecutorService) extends Specification { def is = "test" ! Await.result(Future(ok)(ExecutionContext.fromExecutor(es)), 1 second) }
  ...
  case class SpecM2()(using ee: ExecutionEnv)     extends org.specs2.mutable.Specification { "test" >> Await.result(Future(ok)(ee.executionContext), 1 second) }
  case class SpecM3()(using ec: ExecutionContext) extends org.specs2.mutable.Specification { "test" >> Await.result(Future(ok), 1 second) }
  case class SpecM4()(using es: ExecutorService)  extends org.specs2.mutable.Specification { "test" >> Await.result(Future(ok)(ExecutionContext.fromExecutor(es)), 1 second) }
