...
import org.specs2.specification.core.{Env, FormDescription}
import org.specs2.specification.dsl.mutable.{FormDsl, MutableFragmentBuilder}

class FormDslSpec(env: Env) extends org.specs2.Spec with TypedEqual { def is = s2"""
  simple test for the form dsl $e1

"""

  def e1 = {
    DefaultExecutor.executeAll(dsl.insert(Form("test")))(env).head.description match {
