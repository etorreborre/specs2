...
trait FutureMatchers extends Expectations {
   ...
   */
  implicit class futureAsResult[T](f: Future[T])(implicit ec: ExecutionContext, asResult: AsResult[T]) {
  ...
  private def awaitFor[T](m: Matcher[T])(retries: Int = 0, timeout: FiniteDuration = 1.seconds)(implicit ec: ExecutionContext): Matcher[Future[T]] = new Matcher[Future[T]] {
