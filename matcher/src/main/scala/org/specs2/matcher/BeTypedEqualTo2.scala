...
import org.specs2.matcher.describe.{DefaultDifferenceInspector, Diffable, DifferenceInspector}
import org.specs2.text.NotNullStrings._

import scala.collection.{GenTraversable, GenTraversableOnce}


class BeTypedEqualTo2[T : Diffable](t: =>T, equality: (T, T) => Boolean = (t1:T, t2:T) => t1 == t2) extends AdaptableMatcher[T] {
  outer =>

  protected val ok: String => String = identity
  protected val ko: String => String = identity

  protected val differenceInspector: DifferenceInspector = new DefaultDifferenceInspector

  def adapt(f: T => T, okFunction: String => String, koFunction: String => String) = {
    new BeTypedEqualTo2(f(t), equality) {
  ...
  def apply[S <: T](b: Expectable[S]): MatchResult[S] = {
    ...
    val diff = differenceInspector.diff(actual, expected)
