...
trait FutureMatchers extends FutureBaseMatchers:
   ...
   */
  implicit class futureAsResult[T](f: => Future[T])(using ee: ExecutionEnv, asResult: AsResult[T]) extends FutureAsResult[T](f)

private[specs2]
trait FutureBaseMatchers extends ExpectationsCreation:

  def await[T](m: Matcher[T])(using ee: ExecutionEnv): Matcher[Future[T]] = awaitMatcher(m)(retries = 0, timeout = 1.second)
  ...
  private[specs2] def awaitMatcher[T](m: Matcher[T])(retries: Int, timeout: FiniteDuration)(using ee: ExecutionEnv): Matcher[Future[T]] = new Matcher[Future[T]] {
