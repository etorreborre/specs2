...
/** Inherit this trait to provide a Matcher where both the actual and the expected values can be adapted with a
  * function.
  */
trait AdaptableMatcher[T] extends Matcher[T]:
  ...
  /** Adapt a matcher with both the expected and actual values ex: `be_==("message") ^^^ (_.trim)` will do the
    * comparison on both trimmed strings
    */
  ...
  /** Adapts a matcher by changing the actual value before doing the match ex: `be_==("message") ^^ (_.trim)` will do
    * the comparison on a trimmed String before match
    */
  def ^^(f: T => T): AdaptableMatcher[T] =
    new AdaptableMatcher[T]:
