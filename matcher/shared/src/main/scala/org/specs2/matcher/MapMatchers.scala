...
private[specs2]
object MapBaseMatchers extends MapBaseMatchers

private[specs2]
trait MapBeHaveMatchers extends BeHaveMatchers:
  private val outer = MapBaseMatchers

  extension [K](result: MatchResult[Iterable[(K, Any)]])
  ...
  extension [V](result: MatchResult[Iterable[(Any, V)]])
  ...
  extension [K, V](result: MatchResult[Iterable[(K, V)]])
  ...
  extension [K, V](result: MatchResult[PartialFunction[K, V]])
  ...
  def key[K](k: K) = outer.haveKey(k)
  def keys[K](ks: K*) = outer.haveKeys(ks:_*)
  def value[V](v: V) = outer.haveValue(v)
  def values[V](vs: V*) = outer.haveValues(vs:_*)
  def pair[K, V](p: (K, V)) = outer.havePair(p)
  def pairs[K, V](pairs: (K, V)*) = outer.havePairs(pairs:_*)
  def definedAt[K](values: K*) = outer.beDefinedAt(values:_*)
  def definedBy[K, V](values: (K, V)*) = outer.beDefinedBy(values:_*)
