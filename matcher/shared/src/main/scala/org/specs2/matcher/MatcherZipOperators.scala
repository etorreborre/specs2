...
trait MatcherZipOperators extends ExpectationsCreation { outer =>
  ...
  implicit def ContainSeqMatcherFunction[T](seq: Seq[T]): ContainSeqMatcherFunctionOps[T] =
    ContainSeqMatcherFunctionOps(seq)

  class ContainSeqMatcherFunctionOps[T](seq: Seq[T]) {
