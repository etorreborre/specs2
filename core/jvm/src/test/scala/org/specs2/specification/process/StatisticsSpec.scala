...
class StatisticsSpec(ee: ExecutionEnv) extends Specification {
  def is = s2"""
...
"""
  ...
  def e2 =
    ...
    Seq(success, failure, pending, skipped) must contain((r: Result) =>
      foldStats(r) === Stats.empty.withResult(r)
    ).forall
}
