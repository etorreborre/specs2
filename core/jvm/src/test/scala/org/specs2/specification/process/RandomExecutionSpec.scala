...
class RandomExecutionSpec(val env: Env) extends script.Specification with Groups with ThrownExpectations with ExampleDsl with OwnEnv { def is = section("ci") ^ s2"""
...
"""
  ...
  "random" - new group with results {
    eg := {
      ...
      runAction(DefaultExecutor.runSpecificationAction(spec, ownEnv).map { _ =>
