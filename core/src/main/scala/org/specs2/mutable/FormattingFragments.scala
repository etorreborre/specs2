...
// These traits are a workaround for https://issues.scala-lang.org/browse/SI-8010
// The bug will be fixed in 2.11.0-M8
trait FormattingFragments0 extends specification.FormattingFragments { this: FragmentsBuilder =>
  import StandardFragments._
  override def t: Tab      = { val f = super.t    ; addFragments(f); f }
  override def bt: Backtab = { val f = super.bt   ; addFragments(f); f }
}

trait FormattingFragments1 extends specification.FormattingFragments { this: FragmentsBuilder =>
  override def t(n: Int)  = { val f = super.t(n) ; addFragments(f); f }  
  override def bt(n: Int) = { val f = super.bt(n); addFragments(f); f }
}
 

trait FormattingFragments extends FormattingFragments1 with FormattingFragments0 { outer: FragmentsBuilder =>
