            ...
            val scriptFragments =
              fragments.lastOption match {
                case Some(f) if Fragment.isText(f) =>
                  Fragments(fragments.dropRight(1):_*) append script.fragments(f.description.show + text).map(_.setLocation(end))

                case None =>
                  Fragments(fragments:_*) append script.fragments(text).map(_.setLocation(end))
