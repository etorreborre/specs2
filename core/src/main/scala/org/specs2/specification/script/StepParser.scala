...
/** A StepParser is a function to extract a value of type `T` from a piece of text It can also strip the text from
  * delimiters if any
  */
trait StepParser[T] { outer =>

  /** parse some text and extract some well-type value T if the original text contains delimiters to indicate the values
    * to extract, remove them
    *
