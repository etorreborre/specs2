...
import data.HopcroftKarp.*
import execute.*
import BestMatching.*

class HopcroftKarpSpec extends Spec {
  def is = s2"""
...
"""
  ...
  def g1 =
    val graph = Map(1 -> List(6, 7, 8), 2 -> List(), 3 -> List(6), 4 -> List(8), 5 -> List(6, 9))

    findMaximalMatching(1 to 5, 6 to 9, graph) must contain((1, 7), (3, 6), (4, 8), (5, 9))

  def g2 =
    val graph = Map(1 -> List(6, 8, 10), 2 -> List(6), 3 -> List(6), 4 -> List(9), 5 -> List(10))
  ...
  def g3 =
    ...
    val (matches, remaining) =
      findBestMatch[Int, List[Int], Result](
        ...
        (i: Int, list: List[Int]) =>
          if list.contains(i) then ok(s"$list contains $i") else ko(s"$list does not contain $i")
      )

    matches.map { case (i, j, r) => r.message } must contain(
      exactly(
        "List(1) contains 1",
