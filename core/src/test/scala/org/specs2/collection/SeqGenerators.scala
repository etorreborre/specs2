...
trait SeqGenerators:
  /** @return
    *   a Generator for slices of a given sequence of elements.
    *
  ...
  /** @return
    *   a Generator for slices of a some elements out of a given sequence of elements.
    */
  ...
  private def slicesOfElements[T](ts: Seq[T]): Gen[Seq[Seq[T]]] =
    ...
    for indices <- Gen.someOf(ts.indices)
