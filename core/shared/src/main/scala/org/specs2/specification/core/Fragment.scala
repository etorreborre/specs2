...
/** Fragment of a specification
  *
  * It has a description (generally text but sometimes not, for a step for example) It has an execution which might do
  * or don't do anything (for examples it runs some code)
  *
  * By default its location is provided by a stacktrace containing the user file with line number which triggered this
  * fragment creation. The location is however provided by a macro in the case of s2 interpolated strings
  */
...
object Fragment:
  ...
  def specificationRef: PartialFunction[Fragment, SpecificationRef] = { case Fragment(l: SpecificationRef, _, _) => l }

  def marker: PartialFunction[Fragment, Marker] = { case Fragment(m: Marker, _, _) => m }
