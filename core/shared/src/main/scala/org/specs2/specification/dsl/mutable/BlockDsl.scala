...
trait ExtendedBlockDsl extends BlockDsl:
  ...
  /** adding a conflicting implicit to warn the user when a `>>` was forgotten
    */
  implicit def `***If you see this message this means that you've forgotten an operator after the description string: you should write "example" >> result ***`(
      s: String
...
private[specs2] trait BlockCreation extends FragmentBuilder with FragmentsFactory:
