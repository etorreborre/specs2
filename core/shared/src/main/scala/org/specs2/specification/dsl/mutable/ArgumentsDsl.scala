...
/** Methods with default Property values to create Arguments instances Arguments are being added to the
  * SpecificationStructure by mutating its current content
  */
trait ArgumentsCreation extends org.specs2.main.ArgumentsCreation with MutableArgumentsBuilder:
  ...
  private[specs2] class ArgumentsNamespaceMutable extends ArgumentsNamespace:
    ...
    ) = updateArguments(super.select(ex, include, exclude, was, selector))
    ...
    ) = updateArguments(super.store(reset, never))
