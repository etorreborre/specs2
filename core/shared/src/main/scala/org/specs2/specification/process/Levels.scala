...
/** Compute the "level" of each fragment to be able to represent the whole specification as a tree.
  *
  * In mutable specifications text fragments add one level to the following fragments, Otherwise should and can blocks
  * create Start/End fragments indicating that the level should go up then down
  */
trait Levels:
  ...
  def levelsToTreeLoc(mapper: Mapper): AsyncTransducer[(Fragment, Int), TreeLoc[Fragment]] = {
    (p: AsyncStream[(Fragment, Int)]) =>
      val init = Leaf((DefaultFragmentFactory.text("root"), 0)).loc

      p.state[TreeLoc[(Fragment, Int)], TreeLoc[(Fragment, Int)]](init) { case ((f, level), treeLoc) =>
        val parent =
          if level == 0 then treeLoc.root
          else (treeLoc.parentLocs :+ treeLoc).takeWhile(_.getLabel._2 < level).lastOption.getOrElse(treeLoc)
