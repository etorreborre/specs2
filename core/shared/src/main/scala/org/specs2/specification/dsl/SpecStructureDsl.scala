...
trait SpecStructureDsl extends FragmentsFactory:
  ...
  given [T <: SpecificationStructure] => ToSpecStructure[String, T]:
  ...
  given [T <: SpecificationStructure] => ToSpecStructure[Fragment, T]:
  ...
  given ToSpecStructure[SpecHeader, Seq[Fragment]]:
    def toSpecStructure(header: SpecHeader, fragments: =>Seq[Fragment]): SpecStructure =
      SpecStructure.create(header, Arguments(), Fragments(fragments*))
  ...
  given ToSpecStructure[SpecStructure, Seq[Fragment]]:
    def toSpecStructure(structure: SpecStructure, fragments: =>Seq[Fragment]): SpecStructure =
      structure.copy(lazyFragments = () => structure.fragments.append(Fragments(fragments*)))
  ...
  given [T <: SpecificationStructure] => ToSpecStructure[SpecStructure, T]:
  ...
  // deliberately an implicit class and not an extension: an extension would duplicate the `^`
  // defined above, and the lower priority is what lets the two coexist
