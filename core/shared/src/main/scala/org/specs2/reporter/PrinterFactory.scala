...
/** Create printers based on their class names and the arguments passed by the user Note that this operation might fail
  * if the corresponding printer classes are not on the class path
  */
case class PrinterFactory(arguments: Arguments, customInstances: CustomInstances, logger: Logger):
  ...
  def createTextPrinter: Operation[Option[Printer]] =
    if !printerNames.map(_.name).exists(arguments.isSet) || arguments.isSet(CONSOLE.name) then
      Operation.ok(Some(TextPrinter.default))
  ...
  def createHtmlPrinter: Operation[Option[Printer]] =
    customInstances.createPrinterInstance(
      HTML,
      "org.specs2.reporter.DefaultHtmlPrinter",
