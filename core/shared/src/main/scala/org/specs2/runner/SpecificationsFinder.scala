...
/** This trait loads specifications found on a given source directory based on a regular expression representing the
  * Specification name, usually .*Spec
  */
trait SpecificationsFinder:
  ...
  def findSpecifications(
      glob: String = specificationsPath,
  ...
  ): Operation[List[SpecificationStructure]]
...
case class DefaultSpecificationsFinder(env: Env) extends SpecificationsFinder:
        ...
        names
          ...
          .traverse { name =>
            ...
            SpecificationStructure
              .create(name, classLoader, Some(env))
              .map(s => Option(s))
              .orElse(
                logger.warn("cannot create specification " + name + options).as(None: Option[SpecificationStructure])
              )
          }
          .map(_.flatten)
    }
  ...
  /** @param glob
    ...
    *   a function to filter out unwanted specifications
    * @return
    *   specifications links created from the found specifications. When a specification can not be instantiated a
    *   failed example is created for it
    */
  def specificationLinks(
      glob: String = "**/*.scala",
      pattern: String = specificationsPattern,
      filter: String => Boolean = { (name: String) => true },
      basePath: DirectoryPath = DirectoryPath.unsafe(new java.io.File("src/test/scala").getAbsolutePath),
    ...
    val links: Operation[List[Fragment]] = specificationNames(glob, pattern, basePath, verbose).flatMap { names =>
      names.filter(filter).traverse { name =>
        SpecificationStructure
          .create(name, classLoader, Some(env))
          .map(s => link(SpecificationRef.create(s.is)))
  ...
  def readClassNames(
    ...
    yield (sourceFile.classNames(packName, fileContent, objectPattern, "$", verbose) |@| sourceFile.classNames(
      packName,
      fileContent,
      classPattern,
      "",
