...
"""
  ...
  def a2 =
    SpecStructure.linkedSpecifications(s1, env, getClass.getClassLoader).runOption.flatMap(_.lastOption) must beSome((_: SpecStructure).arguments must ===(s2.arguments))
