...
class TreesSpec extends Specification with DataTables with Expectations {
  def is = s2"""
...
"""
  ...
  def pruning1 = pruneAndDraw(tree1, prune) must beTree("0", "|", "`- 2", "   |", "   `- 2")

  def pruning2 = pruneAndDraw(tree2, prune) must beTree("0", "|", "`- 2")
  ...
  def treeLoc2 = tree1.loc.addChild(3).tree.drawTree must beTree("0", "|", "+- 2", "|  |", "|  `- 2", "|", "`- 3")
