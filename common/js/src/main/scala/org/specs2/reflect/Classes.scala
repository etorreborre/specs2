...
trait Classes extends ClassOperations {
  ...
  def createInstance[T <: AnyRef](className: String, loader: ClassLoader, defaultInstances: =>List[AnyRef] = Nil)(implicit m: ClassTag[T]): Operation[T] = {
    if (className.endsWith("$")) Operation.delayed[T](TestUtils.loadModule(className.dropRight(1), loader).asInstanceOf[T])
    else                         Operation.delayed[T](TestUtils.newInstance(className, loader, Nil)(Nil).asInstanceOf[T])
  }
