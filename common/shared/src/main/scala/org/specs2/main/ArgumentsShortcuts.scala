...
trait ArgumentsShortcuts { this: ArgumentsCreation =>
  ...
  /** shortcut to skip all examples when a condition is true. if the condition throws an exception, its stacktrace is
    * *not* printed and all the examples are skipped
    */
  def skipAllIf(condition: =>Boolean): Arguments =
    args(skipAll = ArgProperty(tryo(condition).getOrElse(true): Boolean))
  ...
  def diffs(
      ...
      shortenSize: Int = 5,
      full: Boolean = false,
      seqTriggerSize: Int = 0,
      seqMaxSize: Int = 1000000
  ): Arguments =
    args.report(diffs =
      ArgProperty(SmartDiffs(show, separators, triggerSize, shortenSize, diffRatio, full, seqTriggerSize, seqMaxSize))
    )
