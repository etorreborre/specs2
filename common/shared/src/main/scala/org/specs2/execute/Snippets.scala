...
/** Snippets of code can be extracted from interpolated specification strings.
  ...
  * you need to delimit it with some comments `// 8<-------` (with as many dashes >= 2 as you want)
  *
...
trait Snippets:
  ...
  extension [T](s: Snippet[T])
    ...
    def set(
      ...
      s.copy(params = s.params.copy(trimExpression = trimExpression, cutter = cutter, asCode = asCode, prompt = prompt))
...
object Snippets extends Snippets:
  ...
  def create[T](code: Expr[() => T], params: Expr[SnippetParams[T]])(using
      quotes: Quotes
  )(using t: Type[T], t1: Type[() => T]): Expr[Snippet[T]] =
...
/** Captured snippet of code with: a value of type T, a string representing the expression, captured by a macro, some
  * evaluation and display parameters
  */
case class Snippet[T](
    code: () => T,
...
/** Implementation of a function to cut pieces of code by using some comments as markers
  */
case class ScissorsCutter(cutMarker: String = scissorsMarker, cutMarkerFormat: String = scissorsMarkerFormat)
...
object Snippet:
  ...
  /** RangePos snippets are built by using all the string at the macro application point (it used to be only the
    * "inside" code but that got broken in 2.11.0-M8)
    *
  ...
  def trimRangePosSnippet = (call: String) => {
    ...
    val trimmed =
      if s"$ls*\\{$ls*.*".r.findPrefixOf(expression).isDefined then expression.removeFirst(s"\\{").removeLast(s"\\}")
      else expression

    trimmed.removeAll("/**/;1/**/").trim
  }

  /** trim the expression from start and end accolades if it is a multiline expression + in some case it is necessary to
    * add a dummy expression so that the range position of the captured snippet is correct. /**/;1/**/ is the smallest
    * such expression
    */
  def trimApproximatedSnippet = (expression: String) => {
    expression
      .removeFirst(s"snippet\\s*\\{")
      .removeLast(s"\\}")
  ...
  def markdownCode(
      multilineQuotes: String => String = defaultMultilineMarkdownQuotes,
      singleLineQuotes: String => String = defaultSingleLineQuotes,
      offset: Int = 0
  ) = (original: String, cut: String) => {
