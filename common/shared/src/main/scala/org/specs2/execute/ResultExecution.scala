...
trait ResultExecution { outer =>
  ...
  def execute(result: =>Result) =
    try {
      ...
      case e: java.lang.Error if simpleClassName(e) == "ExpectationError"    => Failure(e.getMessage.notNull, "", e.getStackTrace.toList, details = FromExpectationError)
  ...
  def effectively(result: =>Result): Result =
    try {
      ...
      case e: java.lang.Error if simpleClassName(e) == "ExpectationError"    => throw FailureException(Failure(e.getMessage.notNull, "", e.getStackTrace.toList, details = FromExpectationError))
