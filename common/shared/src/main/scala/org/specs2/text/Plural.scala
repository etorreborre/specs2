...
private[specs2] trait Plural:
  ...
  extension (i: Int)
    ...
    /** @return
      *   a Option with a non-pluralized string describing this quantity if it is greater than 0
      */
    def optInvariantQty(s: String): Option[String] = if i > 0 then Some(i.toString + " " + s) else None

    /** @return
      *   the proper postfix for an ordinal number
      */
...
private[specs2] object Plural extends Plural
