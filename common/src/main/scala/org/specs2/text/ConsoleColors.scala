...
/** This class defines the colors to use to print out text on the Console with defaults as AnsiColors for a dark
  * background console
  */
...
object MappedColors:
  def fromArgs(args: String) =
    ...
    if args.contains("whitebg") then
      new MappedColors(Map(map*)) { override lazy val defaultColors = new InvertedColors }
