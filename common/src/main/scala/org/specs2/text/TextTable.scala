...
case class TextTable(header: Seq[String], lines: Seq[Seq[String]], separator: String = "|"):
  ...
  /** @return
    *   a seq of lines where each cell is right-padded with the maximum size of the column cells
    */
  ...
  /** @return
    *   a line where each cell is right-padded with the maximum size of the corresponding column
    */
