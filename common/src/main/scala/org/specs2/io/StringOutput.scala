...
trait StringOutput extends Output:
  ...
  /** if a message is printed with a newline it is considered as being a new message otherwise it is added to the last
    * message
    */
  override def printf(s: String, args: Any*): Unit =
    val formatted = s.format(args*)
    if formatted.endsWith("\n") then append(formatted.dropRight(1))
    else if msgs.isEmpty then append(formatted)
    else
      val last = msgs.last
