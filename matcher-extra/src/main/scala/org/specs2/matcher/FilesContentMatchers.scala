...
/** This trait provides matchers to check the presence of some expected files vs the actual ones and also ways to verify
...
trait FilesContentMatchers
  ...
  /** check that all the paths in `expectedDir` are the same as the ones in `actualDir` then check that all the files in
    * `expectedDir` have the same contents as the files in `actualDir` when they are present
    */
  ...
  def haveSameMD5: Matcher[(File, File)] = { (pair: (File, File)) =>
    ...
    yield
      val message = TextTable(
        header = Seq("file", "MD5"),
        lines = Seq(Seq(actual.getPath, md5_1), Seq(expected.getPath, md5_2))
      ).show
      (md5_1 == md5_2, s"MD5 mismatch:\n$message")
    operation.runOperation.fold(error => (false, execute.Error(error).message), identity)
  }
  ...
  case class LocalFilesContentMatcher(
      expectedDir: File,
      filter: File => Boolean = (f: File) => true,
      filesMatcher: Matcher[(File, File)] = haveSameLines[File, File]
  ) extends Matcher[File]:

    def apply[S <: File](actualDir: Expectable[S]) =
      val expectedFiles = LocalPaths(DirectoryPath.unsafe(expectedDir), filePathFilter(filter))
      val pairs = expectedFiles.files
        .map { p =>
          ((DirectoryPath.unsafe(actualDir.value) / p).toFile, (DirectoryPath.unsafe(expectedDir) / p).toFile)
        }
        .filter(_._1.exists)
  ...
  case class LocalPathsAndFilesContentMatcher(
      expectedDir: File,
      filter: File => Boolean = (f: File) => true,
      filesMatcher: Matcher[(File, File)] = haveSameLines[File, File]
  ) extends Matcher[File]:
  ...
  private case class LocalPaths(base: DirectoryPath, filter: FilePath => Boolean):
    def files =
      FilePathReader.listFilePaths(base).map(_.filter(filter).map(_.relativeTo(base)).sortBy(_.path)).runMonoid
