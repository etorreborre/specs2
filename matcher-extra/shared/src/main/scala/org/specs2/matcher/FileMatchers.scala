...
trait PathBeHaveMatchers extends BeHaveMatchers { outer: PathBaseMatchers =>
   ...
   */
  implicit def toPathResultMatcher(result: MatchResult[String]): PathResultMatcher = new PathResultMatcher(result)
...
trait FileBeHaveMatchers extends BeHaveMatchers { outer: FileBaseMatchers =>
   ...
   */
  implicit def toFileResultMatcher[T <: { def getPath(): String }](result: MatchResult[T]): FileResultMatcher[T] = new FileResultMatcher(result)
