...
/** A Form is a container for Rows (@see Row) where each row contain some Cell (@see Cell). It has an optional title and
  ...
  */
class Form(val title: Option[String] = None, val rows: Seq[Row] = Vector(), val result: Option[Result] = None)
    extends Executable
    with Text:
  ...
  def th(h1: Field[?], hs: Field[?]*): Form = tr(
    Row.tr(FieldCell(h1.header), hs.map((f: Field[?]) => FieldCell(f.header))*)
  )
  ...
  def executeForm: Form =
    if result.isDefined then this
      ...
      newForm(
        title,
        executedRows,
        Some(executedRows.map(_.execute).foldLeft(success: Result) { (res, cur) => res and cur })
      )
  ...
  def toProp(label: String): Prop[Form, Any] =
    ...
    Prop(label, executed, (f: Form, s: Any) => executedResult).apply {
      if executedResult.isSuccess then "success": Any
      else Form.toXml(executed)(using Arguments())
    }
...
case object Form:
  ...
  /** Private methods for building the Form xml
    */
  private def title(form: Form, colnumber: Int) =
    form.title.map(t => <tr><th colspan={(colnumber + 1).toString}>{t}</th></tr>).toList.reduceNodes
